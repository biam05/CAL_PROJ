...
A aplicação \textit{WasteApp} desenvolvida pelo nosso grupo de trabalho tem como base uma interface com a qual o utilizador poderá interagir. Inicialmente, o utilizador terá selecionar se pretende fazer \textit{login} na sua conta ou criar uma conta nova. Estas opções são necessárias uma vez que o utilizador poderá ter um utilizador "normal" que tem como objetivo saber qual o ponto de recolha mais próximo de sua casa ou um utilizador que pretende ir a casa dos restantes recolher os resíduos, logo terão de ser apresentados menus diferentes com funcionalidades igualmente diferentes.

A informação relativamente aos pontos de interesse é guardada num grafo, sendo que estes são os vértices (nem todos os vértices são pontos de interesse, no entanto todos os pontos de interesse são vértices). Este grafo é gerado e apresentado no ecrã do utilizador com o auxílio da API \textit{GraphViewer}, sendo que este é atualizado ao longo do programa de forma a ser possível a visualização dos diferentes trajetos selecionados pelo utilizador e diferentes pontos de recolha disponíveis para o utilizador.

{\Large ALTERAR: PODERÁ TER ERROS; REVER!!!}
...
A classe \textbf{\textit{House}}, definida em \textit{house.h}, 

{\Large COMPLETAR AQUI COM A NOVA INFORMAÇÃO.}

\subsection{Ponto de Recolha}

A classe \textbf{\textit{Spot}}, definida em \textit{spot.h}, 
