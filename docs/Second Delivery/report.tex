...
Por outro lado, a \textbf{complexidade espacial} tem como valor O($|V|$) uma vez que foi utilizado um vetor na implementação da \textit{priority queue}, levando assim a uma \textbf{complexidade espacial linear}.

\subsection{Análise temporal empírica }

\begin{figure}[h!]
  \centerline{\includegraphics[scale=0.6]{dijkstra_empirica.png}}
  \caption{Complexidade temporal empírica do algoritmo de \textit{Dijsktra}.}
\end{figure}

Após vários testes utilizando a \textit{library} \textit{chrono}, conseguimos determinar uma média dos tempos de execução deste algoritmo para os mapas \textit{4x4}, \textit{8x8} e \textit{16x16}, sendo estes valores apresentados na tabela que se segue. No entanto, aquando da verificação destes, notamos que os valores não seriam os esperados tendo em conta a complexidade temporal deste algoritmo, uma vez que utilizamos estruturas de dados que possivelmente não seriam as mais corretas para o algoritmo em questão. Destas temos como exemplo vetores que provavelmente deveriam ter sido de \textit{pointers} e não dos \textit{vertexes} em si, o que acabaria por tornar o algoritmo bem mais eficiente. 
...
\begin{figure}[h!]
  ...
  \caption{Complexidade temporal empírica do algoritmo de \textit{Held Karp}.}
\end{figure}

Tal como fizemos com o algoritmo de \textit{Dijkstra}, realizamos vários testes com o auxílio da \textit{library} \textit{chrono}, determinando assim uma média de tempos de execução do algoritmo de \textit{Held-Karp} para os mapas \textit{4x4}, \textit{8x8} e \textit{16x16} e chegamos novamente à conclusão de que os resultados obtidos divergiam dos resultados esperados. Esta diferença deve-se, não só, ao uso de estruturas de dados que não são as apropriadas para o algoritmo em questão como também a utilização de um \textit{Dijkstra} durante esta para auxiliar no cálculo do caminho mais curto entre casas, levando a um aumento significativo dos tempos de execução.
...
\begin{figure}[h!]
  \centerline{\includegraphics[scale=0.3]{conectividade.png}}
  \caption{Grafo \textit{4x4} dirigido.}
\end{figure}

Por outro lado, ao tentar aplicar este algoritmo no mapa do Porto, não foi retornado nenhum \textit{output} provavelmente pelo facto do mapa ser demasiado desconexo e demorar demasiado tempo a determinar. Deste modo, para conseguirmos testas o programa, tivemos de colocar os pontos de interesse com distâncias pequenas entre si de forma a arranjarmos zonas relativamente conexas. 

\begin{figure}[h!]
  \centerline{\includegraphics[scale=1]{porto_disconnected.png}}
...
Figura 2 e Figura 12: Wikipedia Contributors (2019). \textit{Held–Karp algorithm}. [online] Wikipedia. Disponível em: \url{https://en.wikipedia.org/wiki/Held%E2%80%93Karp_algorithm}.
